...
} elseif($method == "users_page" && $page > 0) {
    $result = $mysqli->query("SELECT * FROM users");
    $users_array = $result->fetch_all();
    for ($i = ($page*10)-10; $i < $page*10; $i++) {
        if($i < count($users_array)) {
            $users[$i] = array("full_name" => $users_array[$i][2] . " " . $users_array[$i][2],
                "native" => $users_array[$i][6],
                "gender" => $users_array[$i][5],
                "platform" => "FBM",
                "platform_id" => "abcede12345",
                "first_seen" => $users_array[$i][8]);
        }
    }
...
//http://localhost/123/index.php?auth=wr67uythjty65e6hjrtdh4wy&method=users_page&page=1
